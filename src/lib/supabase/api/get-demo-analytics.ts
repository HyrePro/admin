...
export async function getDemoAnalytics(
  jobId: string
): Promise<{ data: DemoAnalytics | null; error: string | null }> {
  ...
  try {
    ...
    const response = await fetch(`/api/demo-analytics?jobId=${jobId}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.error || 'Failed to fetch demo analytics');
    }
    ...
    return { data: data as DemoAnalytics, error: null };
  } catch (err) {
    console.error('Error fetching demo analytics:', err);
    return {
      data: null,
      error: err instanceof Error ? err.message : 'An unexpected error occurred while fetching demo analytics',
    };
  }
}