...
interface InterviewsPageInitialFilters {
  schoolId: string;
  userId: string;
...
export default function InterviewsPageClient({
  ...
  initialFilters: InterviewsPageInitialFilters | null;
}) {
  ...
  const [activeFilter, setActiveFilter] = useState<InterviewStatusFilter>(
    initialFilters?.statusFilter ?? "all",
  );
  const [assignedToMe, setAssignedToMe] = useState(initialFilters?.jobsAssignedToMe ?? false);
  const [isPanelist, setIsPanelist] = useState(initialFilters?.panelist ?? false);
  const [filterPopoverOpen, setFilterPopoverOpen] = useState(false);
  
  // State for calendar view and current date
  const [view, setView] = useState<InterviewCalendarView>(initialFilters?.view ?? "week");
  const [currentDate, setCurrentDate] = useState(
    new Date(initialFilters?.currentDate ?? new Date().toISOString().split("T")[0]),
  );
  
  // Loading states for filter changes
  const [isFiltering, setIsFiltering] = useState(false);

  const schoolId = initialFilters?.schoolId ?? "";
  const userId = initialFilters?.userId ?? "";
  ...
  useWarmRoute("warm_interviews", hasLoadedOnce, 1800);
  ...
  useEffect(() => {
    ...
    if (isPanelist) params.set("panelist", "true");
  ...
  // Type definition
  type StatusKey = InterviewStatusFilter;
  ...
  const ResponsiveInterviewFilters = ({
    ...
    activeFilter: InterviewStatusFilter;
    setActiveFilter: (filter: InterviewStatusFilter) => void;
      ...
      <div className="flex items-center justify-between gap-3 w-full">
        ...
        <Popover open={filterPopoverOpen} onOpenChange={setFilterPopoverOpen}>
          ...
          <PopoverContent align="end" className="w-72 p-4 space-y-4">
            ...
            {/* CTA */}
            <div className="pt-2">
              <Button className="w-full" size="sm" onClick={() => setFilterPopoverOpen(false)}>
                Apply Filters
              </Button>
            </div>
  ...
  if (!initialFilters) {
    return (
      <div className="flex flex-1 items-center justify-center p-8 text-center text-muted-foreground">
        Unable to load interview filters for this account.
      </div>
    );
  }
